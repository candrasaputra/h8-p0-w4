...
function countProfit(shoppers) {
    ...
    for (let i = 0; i < listBarang.length; i++) {
        ...
        hit.product = listBarang[i][0];
        hit.shoppers = [];
        hit.leftOver = listBarang[i][2];
        hit.totalProfit = 0;

        for (let j = 0; j < shoppers.length; j++) {
            if (listBarang[i][0] === shoppers[j].product) {
                if (hit.leftOver >= shoppers[j].amount) {
                    hit['shoppers'].push(shoppers[j].name);
                    hit.leftOver -= shoppers[j].amount;
                    hit.totalProfit += listBarang[i][1] * shoppers[j].amount;
                }
            }
        }
